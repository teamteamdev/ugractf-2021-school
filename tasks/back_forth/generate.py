...
PREFIX = "ugra_obscure_unix_security_"
SECRET = b"inch struggle disappear design"
...
def generate():
    ...
    data = open(os.path.join("private", "secure_chat"), "rb").read()
    data = replace_placeholder(data, b"PREFIX", prefix)
    data = replace_placeholder(data, b"SUFFIX", suffix)
    with open(os.path.join(target_dir, "secure_chat"), "wb") as out:
        out.write(data)
